...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            positionProb = float("-inf")
            mostLikely = None
            for ghost in distribution:
                if distribution[ghost] > positionProb:
                    positionProb = distribution[ghost]
                    mostLikely = ghost
            if self.distancer.getDistance(pacmanPosition, mostLikely) < closestDistance:
                closestDistance = self.distancer.getDistance(pacmanPosition, mostLikely)
        ...
        for action in legal:
            ...
            if self.distancer.getDistance(nextPosition, closestGhost) < closestDist:
                closestDist = self.distancer.getDistance(nextPosition, closestGhost)
                bestMove = action
